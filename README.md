...
python -m scripts.generate --text_prompt "a person waves with his right hand." --motion_length 4  --opt_path /data/wenshuo/project/StableMoFusion/checkpoints/t2m/t2m_condunet1d_batch64/opt.txt
...
```shell 
nohup python -m scripts.train --name freet2m_1 --model-ema --dataset_name t2m
```
</details>
...
```shell
python -m scripts.evaluation --opt_path /data/wenshuo/project/jhz/freet2mnew/StableMoFusion/checkpoints/t2m/freet2m_0/opt.txt  --gpu_id 0
```
...
```shell
python -m scripts.evaluation --opt_path /data/wenshuo/project/StableMoFusion/checkpoints/t2m/t2m_condunet1d_batch64/opt.txt --gpu_id 7 --replication_times 10
```
