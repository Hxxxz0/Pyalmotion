...
class CondUnet1D(nn.Module):
    ...
    def __init__(
            ...
            dropout=0.1,
            no_eff=False,
            physics_dim=29,  # Total physics dimensions for alignment
        ...
        # Physics projection head for representation alignment
        # Projects from mid_dim features to physics space before mid_block1
        self.physics_head = nn.Sequential(
            nn.Conv1d(dims[-1], dims[-1] // 2, kernel_size=1),  # Reduce dimension
    ...
    def forward(
        ...
        # Extract physics features before mid_block1 for representation alignment
        physics_features = None
        if return_physics:
            # Project to physics space using the current deep features
            physics_features = self.physics_head(x)  # [B, physics_dim, T]
...
class T2MUnet(nn.Module):
    ...
    def __init__(self,         
                ...
                activation="gelu", 
                num_text_layers=4,
                physics_dim=29,  # Physics dimensions for alignment
        ...
        # text encoder
        self.embed_text_1 = nn.Linear(qwen_dim, 512)  # 第一步：1024 -> 512
        self.embed_text_2 = nn.Linear(512, text_latent_dim)  # 第二步：512 -> 256
        self.qwen_model_name = qwen_model_name
        self.qwen_model, self.qwen_tokenizer = self.load_and_freeze_qwen(qwen_model_name)
    ...
    def load_and_freeze_qwen(self, qwen_model_name):
        ...
        if qwen_tokenizer is None:
            qwen_tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen3-0.6B-Base", trust_remote_code=True)
    ...
    def encode_text(self, raw_text, device):
        with torch.no_grad():
            # 使用 Qwen3-Embedding 模型进行文本编码（保留token级序列，不做平均池化）
        ...
        # 变换为 TransformerEncoder 期望的 [seq_len, batch, dim]
        x = token_embeddings.permute(1, 0, 2)  # [seq_len, batch, 1024]
        x = self.embed_text_1(x)               # [seq_len, batch, 512]
        x = self.embed_text_2(x)               # [seq_len, batch, 256]
        x = self.textTransEncoder(x)           # [seq_len, batch, 256]
        x = self.text_ln(x)                    # [seq_len, batch, 256]
        # 输出为 [batch, seq_len, dim]
